...
def make_multimethod_aggregate_plot(
    ax,
    aggregate_scores,
    guide,
    ds_counts,
    ...
    method1=" MAST logFC",
    method2="",
    method1_label="SCEPTRE logFC",
    method2_label="BBR Regression Weights",
    kd_method="Full KD",
    pointsize=250,
):
    """
    ...
    markers, caps, bars = ax.errorbar(
        x=df[f"{guide}{method1}"]["mean"],
        y=df[f"{guide}{method2}"]["mean"],
        ls="none",
        xerr=df[f"{guide}{method1}"]["std"],
        yerr=df[f"{guide}{method2}"]["std"],
        fmt="none",
    )
    ...
    if high_count:
        ...
        b = sns.scatterplot(
            x=df_2[f"{guide}{method1}"]["mean"],
            y=df_2[f"{guide}{method2}"]["mean"],
            hue=df_2["mean_TPM"]["mean"],
            zorder=2,
            s=pointsize,
            palette="magma",
            ax=ax,
    ...
    ax.set_ylabel(
        method2_label if prior is None else f"{method2_label}\n Prior = {prior}",
        fontsize=50,
    )
...
def read_bbr_sceptre_mast_results(run_num, kd_percentage, kd_type, ds_type, prior="p5"):
    ...
    """
    sceptre_results_table = pd.read_csv(
        f"D0_perm_ds_{kd_percentage}/{kd_type}/{ds_type}/sceptre_results/sceptre_lowmoi_{kd_percentage}_kd_{run_num}_discovery.csv"
    )
    # Prior 0.5 data
    ...
    bbr_results = pd.read_csv(
        f"D0_perm_ds_{kd_percentage}/{kd_type}/{ds_type}/bbr_results/ds_{file_lab}_results_{prior}_{run_num}.csv",
        index_col=0,
    ).set_index("gene")
    sceptre_logfc = sceptre_results_table.set_index("response_id")[
        ["grna_group", "log_2_fold_change"]
    ].pivot(columns="grna_group")["log_2_fold_change"]
    sceptre_pvalue = sceptre_results_table.set_index("response_id")[
        ["grna_group", "p_value"]
    ].pivot(columns="grna_group")["p_value"]
    sceptre_logfc = bbr_results[["mean_TPM"]].merge(
        sceptre_logfc, left_index=True, right_index=True
    )

    mast_results = pd.read_csv(
        f"D0_perm_ds_{kd_percentage}/{kd_type}/{ds_type}/MAST_results/ds_100_mast_result_{run_num}.csv"
    )
    mast_logfc = (
        mast_results[["gene", "logFC", "Guide"]]
        .pivot(index="gene", columns="Guide")["logFC"]
        .add_suffix("_MAST")
    )
...
def make_aggregate_df(KD_TYPE, percent, ds_type, threshold_3=False, pval_filter=False):
    ...
    for num in np.arange(1, 11):
        (
            sceptre_logfcs[num],
            sceptre_pvalues[num],
            bbr_results_tables[num],
            mast_logfc[num],
        ) = read_bbr_sceptre_mast_results(num, percent, KD_TYPE, ds_type, prior="p1.0")

    # generate a pvalue mask for sceptre, requiring at least 5 significant pvalues
    sceptre_gene_mask_5_sig = (pd.concat(sceptre_pvalues) < 0.05).reset_index(
        1
    ).groupby("response_id").sum() >= 5
    # generate value mask for sceptre, which requires just at least 3 non-nan returns for a gene
    sceptre_gene_mask_3_nonnan = (
        pd.concat(sceptre_logfcs).reset_index().groupby("level_1").count() >= 3
    ).drop(columns=["level_0", "mean_TPM"])

    for num in np.arange(1, 11):
        assert sceptre_logfcs[num].columns[0] == "mean_TPM"
        if pval_filter:
            # filter sceptre information based on pvalues
            sceptre_logfcs[num].iloc[:, 1:] = (
                sceptre_logfcs[num].iloc[:, 1:].where(sceptre_gene_mask_5_sig, np.nan)
            )
        # TODO: decide if this is elif or if--do we want these to be separate options or can a user possibly do both?
            ...
            sceptre_logfcs[num].iloc[:, 1:] = (
                sceptre_logfcs[num]
                .iloc[:, 1:]
                .where(sceptre_gene_mask_3_nonnan, np.nan)
            )
        sceptre_bbr_merge = sceptre_logfcs[num].merge(
            bbr_results_tables[num][compare_cols],
            left_index=True,
            right_index=True,
            suffixes=("_sceptre", "_bbr"),
        )
        merged_dfs[num] = sceptre_bbr_merge.merge(
            mast_logfc[num], left_index=True, right_index=True
        )

    stacked_df = (
        pd.concat(merged_dfs, ignore_index=False)
        .reset_index(1)
    ...
    one_kd_counts = sc.read_h5ad(
        f"D0_perm_ds_{percent}/{KD_TYPE}/{ds_type}/ds_{file_lab}_1.h5ad"
    )
...
def generate_figure():
    ...
    aggregate_bbr_sceptre_50, one_kd_counts_50 = make_aggregate_df(
        KD_TYPE="full_kd",
        percent="50",
        ds_type="ds_100_uniform_sampling",
        pval_filter=True,
    )
    aggregate_bbr_sceptre_25, one_kd_counts_25 = make_aggregate_df(
        KD_TYPE="full_kd",
        percent="25",
        ds_type="ds_100_uniform_sampling",
        pval_filter=True,
    )
    aggregate_bbr_sceptre_15, one_kd_counts_15 = make_aggregate_df(
        KD_TYPE="full_kd",
        percent="15",
        ds_type="ds_100_uniform_sampling",
        pval_filter=True,
    ...
    xmin, xmax, ymin, ymax = make_multimethod_aggregate_plot(
        axs[0],
        aggregate_bbr_sceptre_50,
        "SOX2",
        one_kd_counts_50,
        kd_keep=0.5,
        prior=1,
    ...
    fig.suptitle(
        "Simulated Knockdown Method Comparison, \nwith 5 significant sceptre pvalues thresholding",
        fontsize=70,
        y=1.15,
    )
    fig.legend(
