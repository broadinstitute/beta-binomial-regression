...
def get_pvalues_df(second_deriv, w, features_order, genes, cc=True):
    ...
    pvalues_df = pd.DataFrame(pvalues.T, columns=np.hstack((features_order, ["S_score", "G2M_score"])) if cc else features_order, index=genes)

    return pvalues_df


def run_whole_bbr(counts, group_name, split=False, priorval=.1, genelist=None, permuted=False, no_ds_counts=None, cc=True, maxiter=3000):
    ...
    if split:
        ...
        regression_output = sgd_optimizer(counts, a_NC, b_NC, lr=0.001, maxiter=maxiter, priorval=priorval, subset=True, genelist=counts.var_names[0:5000], permuted=permuted, cc=cc)
        regression_output_2 = sgd_optimizer(counts, a_NC, b_NC, lr=0.001, maxiter=maxiter, priorval=priorval, subset=True, genelist=counts.var_names[5000:], permuted=permuted, cc=cc)
    ...
    # if we aren't splitting, we are either running it on the full dataset or passing a specific gene list to run on, withpermutation specified as well
    # for example, genelist could be the downsampled genes only

    elif genelist is not None:
        regression_output = sgd_optimizer(counts, a_NC, b_NC, lr=0.001, maxiter=maxiter, priorval=priorval, subset=True, genelist=genelist, permuted=permuted, cc=cc)
        weights, _, _, _, _, features, second_deriv, loss_plt, features_order = regression_output
        subset_counts = counts.copy()[:, genelist]
        pvalues_df = get_pvalues_df(second_deriv, weights, features_order, genelist, cc=cc)
        bbr_df = make_bbr_df(weights, subset_counts, features_order, cc=cc, subset=True, genelist=genelist, orig_counts=counts if no_ds_counts is None else no_ds_counts)
        return bbr_df, pvalues_df

    else:
        regression_ouptut = sgd_optimizer(counts, a_NC, b_NC, lr=0.001, maxiter=maxiter, priorval=priorval, permuted=permute, cc=cc)
        weights, _, _, _, _, features, second_deriv, loss_plt, features_order = regression_output


    pvalues_df = get_pvalues_df(second_deriv, weights, features_order, counts.var.index, cc=cc)
