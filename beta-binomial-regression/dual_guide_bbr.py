...
def main():
    ...
    parser.add_argument("-c", dest="counts_h5",
                        help="10x h5 filtered counts", type=str, required=True)
    parser.add_argument("-f", dest="feature_calls",
                        help='protospacer_calls_per_cell csv file, with cells and their feature calls', type=str, required=True)
    ...
    crispr_keep['gene'] = crispr_keep.feature_call.str.replace(pattern, '', regex=True).str.split('_').str[-1]
    ...
    print("Basic filtering of counts for mito, ribo, min number of expressed genes/cells. \n Min genes=200, Min cells=20.")
    cts_keep_filtered = filter_counts(cts_keep.copy())

    bg_set = crispr_keep[(crispr_keep.feature_call.str.startswith('dgA_NC')|crispr_keep.feature_call.str.startswith('dgA_J-NC'))]
    ...
    cts_keep_filtered.obs['target'] = [x if 'NC' not in x else 'NC' for x in cts_keep_filtered.obs.gene]

    print("Generate features matrix.")
    # cc always false here
    torch.set_default_tensor_type('torch.cuda.FloatTensor')
    # do not delete NC names unless specifying
    feature_names, features, feature_df = generate_features_generic(cts_keep_filtered, delete_names=None, column='target', cc=False)
    ...
    regression_output = sgd_optimizer(cts_keep_filtered, a_NC, b_NC, lr=0.001, features=features,
                                    features_order=feature_names, maxiter=1000,
                                    priorval=1, subset=True, genelist=genelist, permuted=False, cc=False,
                                    sparse=True)
    ...
    pvalues_df = get_pvalues_df(second_deriv, weights, features_order, genelist, cc=False)
    bbr_df = make_bbr_df(weights, subset_counts, features_order, cc=False, subset=True, genelist=genelist, orig_counts=cts_keep_filtered)
